...
def get_result(image, label, net, test_save_path, case_name):
    with torch.no_grad():
        pred = net(image.float().cuda())
        out = torch.argmax(torch.softmax(pred, dim=1), dim=1).squeeze(0)
        save_image(make_grid(out, nrow=1).float(), test_save_path + f"/{case_name}.png")
    return [
        calculate_metric_percase(
            out.cpu().detach().numpy() == 1,
            label.squeeze(0).cpu().detach().numpy() == 1,
        )
    ]
